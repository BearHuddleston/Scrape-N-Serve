...
switch_to_react() {
  ...
  # Use sed to replace the frontend context
  sed -i 's|context: ./frontend-react-native|context: ./frontend|g' "$COMPOSE_FILE"
...
switch_to_react_native() {
  ...
  # Use sed to replace the frontend context
  sed -i 's|context: ./frontend|context: ./frontend-react-native|g' "$COMPOSE_FILE"
...
case "$1" in
  react)
    ...
    ;;
  react-native)
    if grep -q "context: ./frontend-react-native" "$COMPOSE_FILE"; then
